...
%     J_inv = J_pos'* InvertMatrix(J_pos*J_pos'); % Task 1 Pseudo Inverse
%     J_2 = J_wx*(eye(6) + J_inv*J_pos);
%     J_inv2 = J_2'* InvertMatrix(J_2*J_2'); % Task 2 Pseudo Inverse
%     % Second task formulation    
%     q_ = J_inv*error + J_inv2*(error2' - J_wx*J_inv*error) + (eye(6) - J_inv*J_pos)*(eye(6) - J_inv2*J_2)*zeros(6,1);
end
